...
Proof.
  ...
  all: try apply translate_pattern__type_of_GEdgeT; try assumption.
...
Module EvalQueryImpl (S : ExecutionPlan.Spec) : EvalQuery.Spec.
  ...
  Theorem match_clause_type graph pi table'
    ...
    (Hwf : Pattern.wf pi) :
      BindingTable.of_type table' (PatternT.type_of Full pi).
  Proof.
  ...
  Lemma matches_pattern_type_exclude_Into pi pe pv r'
    ...
    (Htype : Rcd.type_of r' = PatternT.type_of Full (Pattern.hop pi pe pv))
    (HIn : In (Pattern.vname pv) (Pattern.dom_vertices pi)) :
      Rcd.type_of (Pattern.ename pe !-> None; r') = PatternT.type_of Full pi.
  Proof.
    assert (Hwf': Pattern.wf pi) by (eauto with pattern_wf_db).
