Fixpoint compute_pattern (pattern : Pattern.t) (graph : PropertyGraph.t) :=
  ...
  | Pattern.multiedge pattern enames etype low up vnames wname vlabels => GRA_operations.transitive_get_edges pattern enames etype low up vnames wname vlabels graph
  end.
...
Fixpoint compute_clause (clause : Clause.t) (graph : PropertyGraph.t) :=
  ...
  | MATCH patterns => match patterns with
                      | [] => (*Empty relation*)
                      | head :: tail => RA_operations.natural_join (compute_pattern head graph) (compute_clause (MATCH tail) graph)
                      .
...
Fixpoint compute_query (query : Query.t) (graph : PropertyGraph.t) :=
    ...
    | [] => (*Empty relation*)
    | head :: tail => RA_operations.natural_join (compute_clause head graph) (compute_query tail graph)
    end.